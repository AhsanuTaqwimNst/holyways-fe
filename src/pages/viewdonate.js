...
function ViewDonate() {
  ...
  // jmlh yg dh dinmis  = gettransacion
  const getDonate = async () => {
    try {
      const response = await API.get(`/transactions/${id}`);
      setTransaction(response.data.data); // sT : ini list yg success
      if (response.data.data.length !== 0) {
        let datatransaction = response.data.data;
        let datagoal = response.data.data[0].fund.goal;
        console.log("ini yg baru", response.data.data);
        setsubTotal(getSubTotal(datatransaction)); // <==== di ubah krna set
  ...
  const getDonatePending = async () => {
    try {
      ...
      setTransactionPending(response.data.data); // sTP : ini yg not 
      setisLoading(false);
    } catch (error) {
      console.log(error);
      setisLoading(false);
  ...
  //biar jln otomtis this uE
  useEffect(() => {
    getDonate();
    getDonatePending();
  }, [persen, isLoading]); //  < = klo % brubah,,, ngerender atau update lgi yg baru
  return (
    <div className="pt-5 ">
      {isLoading ? (
        <></>
        ...
        <>
          <div className="col-lg-12  row  justify-content-center ">
            ...
            <div
              className="col-lg-5 d-flex flex-column pb-3"
              style={{ height: 350 }}  
            >
              ...
              <h5 className="mt-5 text-center ">
                ...
                gethered from {convertRupiah.convert(fund?.goal)}
              </h5>
              ...
              <div className="text-center justify-self-end mt-auto ">
                <button
                  className="btn btn-danger w-75 text-center font-weight-bold "
                  data-toggle="modal"
                ...
                <div className="modal-body ">
                  <div className="modal-text">
                    <h5 className="text-center">
                      Dana yang terkumpul {convertRupiah.convert(subTotal)}
                    </h5>
                    <h5>Donasi ini masih membutuhkan dana Sebesar</h5>
                    <span className="font-weight-bold text-center">
                      {convertRupiah.convert(fund?.goal - subTotal)}
                    </span>
                  </div>
                </div>
                <div className="modal-footer align-self-end mt-auto"></div>
              </div>
            </div>
          </div>
          ...
          <div className="col-lg-12  row  justify-content-start ">
            ...
            <h3 className="font-weight-bold mt-4 col-lg-10">
              List Donation Success( {transaction?.length} )
            </h3>
          </div>

          <div
            className="row  justify-content-center hidenscroll"
            style={{ overflowY: "scroll", height: "50vh" }}
          >
            {transaction?.map((item) => (
              <Card
                style={{ width: "18rem" }}
                className="col-8 ml-3 mt-2 mb-2  p-3 border-radius"
              >
                <div className="">
                  <h5 className="text-left  pt-2 font-weight-bold">
          ...
          <div className="col-lg-12  row  justify-content-start ">
            <div className="col-lg-2 " />
            <h3 className="font-weight-bold mt-4 col-lg-10">
              List Donation Not Success( {transactionPending?.length} )
            </h3>
          </div>
          <div
            className="row  justify-content-center hidenscroll"
